...
COR_CAMINHO_A_STAR = (255, 223, 0)

# Inicialização do PyGame
pygame.init()
tela = pygame.display.set_mode((largura_tela, altura_tela))
pygame.display.set_caption("Labirinto - A*")
...
def resolver_a_star(grid):
    ...
    aberta = [(0, 1)]
    g = {inicio: 0}
    f = {inicio: heuristica(inicio, objetivo)}
    pais = {inicio: None}

    while aberta:
        atual = min(aberta, key=lambda x: f[x])
        if atual == objetivo:
            break
        ...
        for dx, dy, parede in direcoes:
            ni, nj = i + dx, j + dy
            if 0 <= ni < cols and 0 <= nj < rows:
                if not celula.paredes[parede]:
                    novo_g = g[(i, j)] + 1
                    if (ni, nj) not in g or novo_g < g[(ni, nj)]:
                        g[(ni, nj)] = novo_g
                        f[(ni, nj)] = novo_g + heuristica((ni, nj), objetivo)
                        pais[(ni, nj)] = (i, j)
                        if (ni, nj) not in aberta:
                            aberta.append((ni, nj))
...
def main():
    ...
    caminho_a_star = resolver_a_star(grid)

    rodando = True
    while rodando:
        tela.fill(COR_FUNDO)
        desenhar_labirinto(grid)
        desenhar_caminho(caminho_a_star, COR_CAMINHO_A_STAR)
        pygame.display.flip()

...
COR_CAMINHO_BFS = (0, 255, 0)
COR_CAMINHO_FORCA_BRUTA = (255, 0, 0)

# Inicialização do PyGame
pygame.init()
tela = pygame.display.set_mode((largura_tela, altura_tela))
pygame.display.set_caption("Labirinto - Aldous-Broder")
...
def resolver_brute(grid):
    ...
    def backtrack(i, j):
        ...
        for dx, dy, parede in direcoes:
            ni, nj = i + dx, j + dy
            if 0 <= ni < cols and 0 <= nj < rows:
                if not celula.paredes[parede] and (ni, nj) not in visitado:
                    if backtrack(ni, nj):
                        caminho.append((i, j))
                        return True
        return False
...
def main():
    ...
    caminho_forca_bruta = resolver_brute(grid)
    caminho_bfs = resolver_bfs(grid)
    ...
    while rodando:
        ...
        desenhar_caminho(caminho_bfs, COR_CAMINHO_BFS)
        pygame.display.flip()
